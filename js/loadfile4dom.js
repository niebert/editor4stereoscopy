/* ---------------------------------------
 ...
 Version:  1.2.21  Date: 2024/07/02 13:40:13
 Homepage: https://gitlab.com/niehausbert/loadfile4dom#readme
 Author:   Engelbert Niehaus
 License:  MIT
 Date:     2024/07/02 13:40:13
 Require Module with:
  ...
  if (pLoader) {
      ...
      if (!this.error_file_type(pLoader,pFileToLoad)) {
        pFileReader.onload = function(fileLoadedEvent){
          ...
          // check if the user wants to have just the file
          // or a filehash with more information about the file e.g. mime_type
          vThis.handle_return_type(pLoader,vJSON,pFileToLoad);
        };
        var vEncoding = "UTF-8";
        if (pLoader.hasOwnProperty("encoding")) {
          vEncoding = pLoader.encoding;
...
LoadFile4DOM.prototype.rows4csv = function (csv,options) {
  options = this.expand_options4csv(options);
  // newline symbol
  var vNewLine = options.newline || "\n";
  // seperator e.g.
  // Comma options.separator=",",
  // Tab options.separator="\t",
  // Pipe options.separator="|"
  var lines=csv.split(vNewLine);

  var result = [];

  var headers=this.get_header4csv(lines[0],options,"row");
  var start = 0;
  ...
  for(var i=start;i<lines.length;i++){
    ...
    var obj = {};
    if ((lines[i]).replace(/[^0-9A-Za-z]/g,"")) {
      var currentline = this.split4line(lines[i],options);
      // remove all
      for(var j=0;j<headers.length;j++){
        obj[headers[j]] = this.convert_decimals2en(currentline[j],options);
      }

      result.push(obj);
    } else {
      console.warn("Line "+(i+1)+"='"+lines[i]+"' ignored!");
    }
  }
...
LoadFile4DOM.prototype.cols4csv = function (csv,options) {
  options = this.expand_options4csv(options);
  // newline symbol
  var vNewLine = options.newline || "\n";
  // seperator e.g.
  ...
  var headers=this.get_header4csv(lines[0],options,"col");
  var start = 0;
  ...
  for (var h = 0; h < headers.length; h++) {
    ...
    // -start because header line is used for header names for the columns
    result[headers[h]]= new Array(lines.length - start);
  }
  for(var i=start;i<lines.length;i++){
    // split the line
    var currentline = this.split4line(lines[i],options);
    // remove all
    for(var j=0;j<headers.length;j++){
      result[headers[j]][i-start] = this.convert_decimals2en(currentline[j]);
    }
  }
...
LoadFile4DOM.prototype.split4line = function (pLine,options) {
  options = this.expand_options4csv(options);
  console.log("options="+JSON.stringify(options,null,4));
  var arr;
  if (pLine) {
    if (pLine.indexOf(options.delimiter) >= 0) {
      // line contains delimiter;
      //console.log("Line '"+pLine+"' with delimiter '"+options.delimiter+"'");
      arr = this.split_line4delimiter(pLine,options);
    } else {
      //console.log("Line '"+pLine+"' WITHOUT delimiter '"+options.delimiter+"'");
      arr = pLine.split(options.separator);
    }
  }
  return arr;
...
LoadFile4DOM.prototype.expand_options4csv = function (options) {
  options = options || {};
  ...
  if (!options.hasOwnProperty("convert2decimals")) {
    options.convert2decimals = true;
  }
  return options;
}

LoadFile4DOM.prototype.get_header4csv = function (pLine,options,pPrefix) {
  options = this.expand_options4csv(options);
  pPrefix = pPrefix || "name";
  var vHeaders = [];
  if (pLine) {
    var splitvals = pLine.split(options.separator);
    for (var i = 0; i < splitvals.length; i++) {
      splitvals[i] = (splitvals[i]).replace(/[^0-9A-Za-z]/g,"");
      var val = (splitvals[i]).replace(/[^0-9,\.\+\-]/g,"");
      if (val.length == splitvals[i]) {
        options.headers = false;
        //alert("options.headers="+options.headers+" due to '"+splitvals[i]+"'")
      } else {
        console.log("Standard header: '"+splitvals[i]+"' options.headers="+options.headers+"")
      }
    }
    if (options.headers == true) {
      //alert("options.headers = true");
      vHeaders = splitvals;
    } else {
      //alert("options.headers = false");
      for (var i = 0; i < splitvals.length; i++) {
        vHeaders.push(pPrefix+(i+1));
      }
    }
  }
  //alert("headers: ['"+vHeaders.join("','")+"']")
  return vHeaders;
}

LoadFile4DOM.prototype.split_line4delimiter = function (pLine,options) {
  options = this.expand_options4csv(options);
  var arr = [];
  var value = "";
  var s, d1, d2;
  while (pLine) {
    ...
    //alert("delimiter='"+pLine+"'")
    d1 = pLine.indexOf(options.delimiter)
    if (d1>=0) {
      pLine = pLine.substring(d1+1);
      d2 = pLine.indexOf(options.delimiter);
      s =  pLine.indexOf(options.separator);
      if (d2>=0) {
        value = pLine.substring(0,d2);
        pLine = pLine.substring(d2+1);
        arr.push(value);
      } else {
        console.warn("second delimiter is missing in '"+pLine+"'");
        // "my value |  rest
        s = pLine.indexOf(options.separator);
        if (s >= 0) {
          // push incomplete string up to separator
          // "my value |  rest -> "my value "
          // trim(...) pushes "my value"
          var val = trim( pLine.substring(0,s));
          val = this.convert_decimals2en(val);
          arr.push(val);
          pLine = pLine.substring(s+1);
        } else {
          arr.push(pLine);
          pLine = "";
        }
...
LoadFile4DOM.prototype.remove_empty_lines = function (pLines,options) {
  options = this.expand_options4csv(options);
...
LoadFile4DOM.prototype.convert_decimals2en = function (pString,options) {
  options = this.expand_options4csv(options);
  var vValue = pString || " ";
  //console.log("convert2decimals('"+pString+"')");
  if ((options.hasOwnProperty("convert2decimals")) && (options.convert2decimals == true)) {
    if (pString) {
      pString = pString+"";
      vValue = pString.replace(/,/g,".");
    } else {
      vValue = "";
    }
  } else {
  ...
  //console.log("convert2decimals('"+pString+"')='"+vValue+"'");
  return vValue;
}

LoadFile4DOM.prototype.check_newline = function (csv,options) {
  options = this.expand_options4csv(options);
  var vNewLine = options.newline || "\n";
  console.log("CSV check_newline():\n"+csv);
  //var csv_ret = csv.replace(/([\n \r]+)/g,"\n");
  var csv_ret = csv.replace(/([\n\r|\r\n|\n|\r]+)/g,vNewLine);
  csv_ret = csv_ret.replace(/\n[ ]+/g,vNewLine);
  console.log("AFTER CSV check_newline():\n"+csv_ret);
  return csv_ret;
    ...
    if (d1>=0) {
      ...
      if (d2>=0) {
        ...
        if (s >= 0) {
          ...
          // "my value |  rest -> "my value "
          // trim(...) pushes "my value"
          arr.push(trim( pLine.substring(0,s)));
          pLine = pLine.substring(s+1);
        } else {
          arr.push(pLine);
          pLine = "";
...
LoadFile4DOM.prototype.handle_csv = function (pLoader,pFileReader,pFileToLoad) {
  var vThis = this; // map "this" to "vThis" for callback inside pFileReader
  if (pLoader) {
      console.log("CALL: handle_csv(''" + pLoader.id + "','" + pLoader.filetype + "')");
      if (!pLoader.options) {
        pLoader.options = {
          "newline":   "\n",
          "separator": ",",
          "delimiter": '"',
          "headers": true,
          "convert2decimals": true
        };
      } else {
        pLoader.options = vThis.expand_options4csv(pLoader.options);
      }
      if (!this.error_file_type(pLoader,pFileToLoad)) {
        pFileReader.onload = function(fileLoadedEvent){
          var vFile = fileLoadedEvent.target.result;
          ...
          try {
              vFile = vFile + "";
              if (vThis.cols4csv && vThis.rows4csv) {
                var c4j = vThis;
                if (pLoader.options) {
                  var lo = pLoader.options;
                  vFile = vThis.check_newline(vFile,lo);
                  if (lo.row2json) {
                    vJSON = c4j.rows4csv(vFile,lo);
                  } else {
                    vJSON = c4j.cols4csv(vFile,lo);
                  }
                }
              } else {
                console.warn("CSV2JSON converter do not exist!");
              }
          } catch (err) {
              vJSON = null; // Init with an empty class
              console.error("ERROR: onload handler with errors - "+err); // error in the above string (in this case, yes)!
...
//#################################################################
//# public Method: handle_dataurl()  Class: LoadFile4DOM
//# Parameter:
//#    pLoader:
//#      contains the LoadFile4DOM loader with all parameters
//#    pFileReader:
...
LoadFile4DOM.prototype.handle_dataurl = function (pLoader,pFileReader,pFileToLoad) {
...
LoadFile4DOM.prototype.handle_file_type = function (pLoader,pFileReader,pFileToLoad) {
  ...
  switch (pLoader.filetype) {
    ...
    case "csv":
      // return just the string of the text file with returntype="file"
      this.handle_csv(pLoader,pFileReader,pFileToLoad);
    break;
    case "dataurl":
      // return just the string of the text file with returntype="file"
      this.handle_dataurl(pLoader,pFileReader,pFileToLoad);
    break;
    case "json":
