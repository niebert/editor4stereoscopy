...
function getExtension4File(pFilename) {
  var vExtension = "";
  if (pFilename) {
    var vPos = pFilename.lastIndexOf(".");
    if (vPos >= 0) {
      vExtension = pFilename.slice(vPos);
    } else {
      console.warn("WARNING: getExtension4File('"+pFilename+"') pFilename does not have an extension");
    }
  } else {
    console.warn("CALL: getExtension4File(pFilename) pFilename does not exist.)");
  }
  return vExtension;
}
